...
mod test {
    ...
    use crate::package::{
        read_address, read_package, write_package, AuthMethodsPackage, AuthSelectPackage,
        ParseResult, PasswordReqPackage, PasswordResPackage, RepliesPackage, RequestsPackage,
        SocksPackage,
    };
    ...
    fn read_requests_package() {
        ...
        let data = [
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x0B, 0xB8,
        ];
        let mut buffer = BytesMut::from(data.as_ref());
        let pac = RequestsPackage::read(&mut buffer);
        assert!(matches!(pac, Ok(ParseResult::Complete(_))));
    ...
    fn write_replies_package() {
        ...
        assert_eq!(
            buffer.as_ref(),
            &[
                0x05, 0x00, 0x00, 0x03, 0x0B, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e, 0x63,
                0x6f, 0x6d, 0x0B, 0xB8
            ]
        );
    }
